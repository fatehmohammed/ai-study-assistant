...
from week4.day16_rag_pipeline import RAGPipeline
...
def convert_pptx_to_pdf(pptx_path: str) -> str:
    """Convert a PPTX file to PDF using LibreOffice. Returns the PDF path."""
    subprocess.run(
...
async def lifespan(app: FastAPI):
    """
    Runs once on server start — initialises the RAG pipeline.
    Same pattern as useEffect(() => {}, []) in React:
    runs once, sets up shared state for all requests.
    """
    global pipeline
...
# Pydantic models = TypeScript interfaces for the API
# They validate incoming JSON and shape outgoing JSON automatically
...
class QuizRequest(BaseModel):
    ...
    question_index: int | None = None  # local index within topic's pre-generated question list
...
def health():
    """
    Health check — tells you the server is running and
    how many chunks are indexed.
    Call this first to verify the server is up.
    """
...
@app.get("/debug")
def debug(query: str = "What causes Parkinsons disease?"):
    """Debug endpoint — calls _retrieve directly and returns raw results."""
    try:
        chunks = pipeline._retrieve(query)
        return {
            "count": pipeline.chunk_count(),
            "retrieved": len(chunks),
            "chunks": [{"score": c["score"], "source": c["source"], "preview": c["text"][:80]} for c in chunks]
        }
...
async def index_pdf(file: UploadFile = File(...)):
    ...
    ⚠️ Edge case: validates PDF file type before processing
    ⚠️ Edge case: cleans up temp file even if indexing fails
    """
    # Validate file type
    ...
    # ⚠️ Edge case: file size limit (10MB)
    ...
    # Save to temp file — PyMuPDF needs a file path, not bytes
    tmp_path = None
    named_path = None
    try:
        ...
        # Rename to original filename so ChromaDB stores the correct source metadata
...
async def index_pdf_stream(file: UploadFile = File(...)):
    """
    Streaming version of /index — emits SSE progress events per page.
    Events:
      {"type": "progress", "page": 3, "total": 66, "pct": 5}
      {"type": "done", "source": "file.pdf", "chunks": 42}
      {"type": "error", "message": "..."}
    """
    if not file.filename.lower().endswith((".pdf", ".pptx")):
    ...
    async def event_stream():
        ...
        try:
            ...
            for i, page in enumerate(doc):
                ...
                if len(text.split()) < 10:
                    # Still unreadable — render to image so user can view it
            ...
            # Save page metadata sidecar so summarize can report coverage
...
def ask(request: AskRequest):
    """
    Ask a question — returns a RAG-grounded answer with sources.

    Body: { "query": "What causes Parkinson's?", "source_filter": null }
    Returns: answer text + list of source chunks used
    """
    ...
    if pipeline.chunk_count() == 0:
        raise HTTPException(
            status_code=400,
            detail="No PDFs indexed yet. Upload a PDF first using POST /index"
        )
...
def get_topics(request: SummarizeRequest):
    """
    Extract major topics and assign every chunk to its best-matching topic.
    Returns [{topic, count}] — count = number of chunks (= planned quiz questions).
    Results cached to disk; both topics and chunk assignments are persisted.
    """
    ...
    # Check cache — must be new format [{topic, count}] with chunk assignment file present
    ...
    # Sample evenly for topic name extraction
...
{context}"""}],
    ...
    # Assign each chunk to its best-matching topic via semantic similarity
    chunk_best: dict[str, tuple[str, float]] = {}  # chunk_id -> (topic, score)
    ...
    # Any unassigned chunk goes to first topic (safety net)
    for chunk_id in all_ids:
        if chunk_id not in chunk_best:
            chunk_best[chunk_id] = (topic_names[0], 0.0)

    # Build topic -> chunk_ids mapping, sorted in lecture order
    ...
    # Result: [{topic, count}] — count is the quiz question budget for that topic
    result_list = [
...
def get_file(filename: str):
    """Serve an uploaded file. For PPTX, serves the converted PDF instead."""
    if filename.lower().endswith(".pptx"):
        pdf_name = os.path.splitext(filename)[0] + ".pdf"
        path = os.path.join(UPLOADS_DIR, pdf_name)
        if not os.path.exists(path):
            # Convert on demand if not already done
...
def get_slide_image(filename: str, page_num: int):
    """Serve a rendered PNG image for a specific page of an indexed file."""
    stem = os.path.splitext(filename)[0]
    path = os.path.join(UPLOADS_DIR, f"{stem}_slides", f"page_{page_num}.png")
    if not os.path.exists(path):
        raise HTTPException(status_code=404, detail=f"Slide {page_num} image not found for '{filename}'")
...
def generate_quiz(request: QuizRequest):
    """
    Generate one MCQ from indexed notes.
    If chunk_index is provided, uses that specific chunk (guaranteed coverage).
    Otherwise falls back to RAG-based retrieval.
    """
    if pipeline.chunk_count() == 0:
        raise HTTPException(status_code=400, detail="No PDFs indexed yet.")

    # Serve from pre-generated question bank when question_index is provided
    ...
    # Try chunk-based approach first
    context = None
    source = None
    evidence_chunks = []
    ...
    if request.chunk_index is not None and request.topic and request.source_filter:
        ...
        if os.path.exists(chunk_assign_path):
            ...
            if request.chunk_index < len(chunk_ids):
                ...
                chunk_result = pipeline._collection.get(
                    ids=[chunk_id],
                    include=["documents", "metadatas"],
                )
    ...
    # Fall back to RAG if chunk-based didn't work
    if context is None:
...
{{
  ...
  "topic": "Short concept name (2–4 words) that this question tests, e.g. Mitosis, Dopamine Pathways"
}}
...
- topic must be a short, specific concept name drawn from the notes content
...
async def generate_questions_stream(request: SummarizeRequest):
    """
    Pre-generate ALL quiz questions for a source.
    For each chunk, Claude generates 1-N questions based on content richness.
    Questions are cached to disk — subsequent calls return instantly.
    SSE events:
      {"type": "progress", "current": 5, "total": 34, "message": "..."}
      {"type": "done", "count": 73, "plans": [{"topic": "...", "count": 5}]}
      {"type": "error", "message": "..."}
    """
    ...
    async def event_stream():
        try:
            ...
            # Flatten all (chunk_id, topic) pairs preserving lecture order within each topic
...
async def summarize_stream(request: SummarizeRequest):
    """
    Map-reduce summarization over ALL indexed chunks for a source.
    Unlike /ask/stream, this does NOT use semantic search — it reads every chunk.
    SSE events:
      {"type": "progress", "current": 1, "total": 4, "message": "..."}
      {"type": "token", "text": "..."}
      {"type": "done", "chunks_processed": 39}
      {"type": "error", "message": "..."}
    """
    if request.source_filter:
        result = pipeline._collection.get(
    ...
    # Sort by chunk_index so batches follow lecture order
    pairs = sorted(
    ...
    BATCH_SIZE = 10  # smaller batches = less content dropped per summary
...
{context}"""}],
            ...
            # Reduce phase: combine + format in one streamed call
...
- Under each section: prose paragraphs weaving in specific facts, numbers, thresholds, drug names, protocols, and dental management points. Include markdown tables for comparisons (e.g. BP categories, angina types, antibiotic regimens)
...
{batch_summaries[0]}"""
            ...
            # Read page metadata sidecar for coverage report
            ...
            if request.source_filter:
                ...
                if os.path.exists(meta_path):
                    ...
                    # Fallback: check slides dir for unreadable pages
                    slides_dir = os.path.join(UPLOADS_DIR, f"{stem}_slides")
...
async def ask_stream(request: AskRequest):
    """
    Streaming version of /ask — sends tokens via Server-Sent Events (SSE).

    Event types:
      {"type": "sources", "sources": [...]}   — sent first, before generation
      {"type": "token",   "text": "..."}      — one per Claude token
      {"type": "done"}                         — signals end of stream
      {"type": "error",   "message": "..."}   — on failure

    Why SSE over WebSockets:
      SSE is one-directional (server → client), which is all we need for
      streaming an AI response. WebSockets add complexity for no benefit here.
    """
    if not request.query.strip():
        raise HTTPException(status_code=400, detail="Query cannot be empty")
    ...
    async def event_stream():
        # 1. Send sources upfront so the UI can render them immediately
        sources = [
            {
                "source": c["source"],
                "score": c["score"],
        ...
        # 2. Build context string (same as _generate)
        context = "\n\n---\n\n".join(
            f"[Source: {c['source']} | Relevance: {c['score']}]\n{c['text']}"
            for c in chunks
        )

        # 3. Stream Claude's response token by token
        # Build messages array: inject notes as first turn, then conversation history
        claude_messages = [
            {"role": "user", "content": f"Notes:\n{context}\n\nQuestion: {request.history[0].text if request.history else request.query}"},
        ]
        # Replay history as alternating user/assistant turns
        if request.history:
            for i, msg in enumerate(request.history):
                if i == 0:
                    # First user message already added above (with notes)
...
If the answer is not in the notes, say so clearly — do not guess.""",
                ...
                for text in stream.text_stream:
                    yield f"data: {json.dumps({'type': 'token', 'text': text})}\n\n"
                    await asyncio.sleep(0)  # flush to client between tokens
